...
// ⚠️ 이 파일은 arm64 인라인 어셈블리로 dyld 의 mmap/fcntl 을 후킹한다.
//    시뮬레이터에는 우회할 라이브러리 검증 자체가 없고 dyld 내부도 달라서 통째로 비운다.
//    (x86_64 시뮬레이터 슬라이스에서는 `svc` 명령이 아예 어셈블되지 않는다)
#if TARGET_OS_SIMULATOR
void init_bypassDyldLibValidation(void) { /* 시뮬레이터에서는 필요 없음 */ }
#else
